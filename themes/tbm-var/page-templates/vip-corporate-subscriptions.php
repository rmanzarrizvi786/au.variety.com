<?php
/*
 * Template Name: VIP - Corporate Subscription
 */

$vcs_submit = filter_input( INPUT_POST, 'vcsSubmit', FILTER_SANITIZE_STRING );
$honeypot   = filter_input( INPUT_POST, 'vip-corporate-subscription-check', FILTER_SANITIZE_STRING );
...
$vip_corporate_subscriptions = PMC\Core\Inc\Larva::get_instance()->get_json( 'modules/vip-corporate-subscriptions.prototype' );

if ( ! empty( $vcs_submit ) ) {
	if ( ! $display_error ) {
		$vip_corporate_subscriptions['vip_corporate_subscriptions_submission_text'] =
...
<form method="POST">
	...
	<?php
	\PMC::render_template(
		sprintf( '%s/template-parts/patterns/modules/vip-corporate-subscriptions.php', untrailingslashit( CHILD_THEME_PATH ) ),
		$vip_corporate_subscriptions,
		true
	);
	?>
