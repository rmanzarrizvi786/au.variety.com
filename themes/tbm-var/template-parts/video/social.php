...
$data = PMC\Core\Inc\Larva::get_instance()->get_json( 'modules/author-social.video' );
...
if ( $is_sponsored ) {
	$sponsor_data               = PMC\Core\Inc\Larva::get_instance()->get_json( 'modules/author-social.video' );
	$data['c_sponsored_social'] = $sponsor_data['c_sponsored']; // Using 'c_sponsored_social' so that is not confused for the standard 'c_sponsored'.
}
...
if ( ! empty( $author_data['single_author'] ) ) {
	...
	if ( ! empty( $author_article_ids ) && is_array( $author_article_ids ) ) {
		...
		foreach ( $author_article_ids as $article_id ) {
			...
			$article_item['c_timestamp']['c_timestamp_text'] = sprintf( '%1$s %2$s', human_time_diff( get_the_time( 'U', $article_id ), current_time( 'timestamp' ) ), __( 'ago', 'pmc-variety' ) ); // @codingStandardsIgnoreLine ignoring since the original template also used 'timestamp' format.
