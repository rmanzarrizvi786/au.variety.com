...
foreach ($lists_data as $key => $list) {
	...
	if (array_key_first($lists_data) === $key) {
		...
		$author_url  = isset($author_data['single_author']) ? get_author_posts_url($author_data['single_author']['author']->ID, $author_data['single_author']['author']->user_nicename) : '#';

		$lists['large_story']['c_link']['c_link_text'] = ''; // "By {$author_data['single_author']['author']->display_name}";
		$lists['large_story']['c_tagline']['c_tagline_text']   = '';
		$lists['large_story']['c_tagline']['c_tagline_markup'] = "By {$author_data['byline']}";
