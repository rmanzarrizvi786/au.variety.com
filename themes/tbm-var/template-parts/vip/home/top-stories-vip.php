...
$data   = PMC\Core\Inc\Larva::get_instance()->get_json( 'modules/top-stories-vip.variety-vip' );
...
$template_1 = $data['top_stories'][0];
$template_2 = $data['top_stories'][1];
$template_3 = $data['o_tease_list']['o_tease_list_items'][0];
...
foreach ( $_posts as $index => $_post ) {
	...
	if ( ! empty( $item_2 ) ) {
		...
		$item_2['c_span']['c_span_link_classes']                = 'u-color-brand-secondary-50 u-letter-spacing-009';
...
\PMC::render_template(
	sprintf( '%s/template-parts/patterns/modules/top-stories-vip.php', untrailingslashit( CHILD_THEME_PATH ) ),
	$data,
	true
);
