	...
	function loadTp() {
		...
		window.tp.push( [
			...
			function() {
				try {
					console.log(
						`PMC Piano tp.push [GA]: [addHandler=${ PIANO_EVENT_CHECKOUT_START }]`
					);
		...
		window.tp.push( [
			'addHandler',
			PIANO_EVENT_PAYMENT_SUBMIT,
			function() {
				try {
					console.log(
						`PMC Piano tp.push [GA]: [addHandler=${ PIANO_EVENT_PAYMENT_SUBMIT }]`
					);
		...
		window.tp.push( [
			'addHandler',
			PIANO_EVENT_CHECKOUT_COMPLETE,
			function( conversion ) {
				try {
					const conversionId =
						window.pmcPianoData.trackingPixels &&
						window.pmcPianoData.trackingPixels.ga &&
						window.pmcPianoData.trackingPixels.ga.conversion_id;
					...
					console.log(
						`PMC Piano tp.push [GA]: [addHandler=${ PIANO_EVENT_CHECKOUT_COMPLETE }, gaConversionId=${ conversionId }]`,
						conversion
					);

					// @TODO send_to needs to be dynamic. This will be handled in PMCS-4438 (https://jira.pmcdev.io/browse/PMCS-4438)
		...
		window.tp.push( [
			...
			function() {
				try {
					const contentName =
						window.pmcPianoData.trackingPixels &&
						window.pmcPianoData.trackingPixels.facebook &&
						window.pmcPianoData.trackingPixels.facebook
							.content_name;

					console.log(
						`PMC Piano tp.push [facebook]: [addHandler=${ PIANO_EVENT_CHECKOUT_START }]`
					);
		...
		window.tp.push( [
			...
			function( conversion ) {
				try {
					console.log(
						`PMC Piano tp.push [Facebook]: [addHandler=${ PIANO_EVENT_CHECKOUT_COMPLETE }]`,
						conversion
					);
		...
		window.tp.push( [
			...
			PIANO_EVENT_CHECKOUT_START,
			function() {
				const checkoutStartPixel =
					window.pmcPianoData.trackingPixels &&
					window.pmcPianoData.trackingPixels.linkedin &&
					window.pmcPianoData.trackingPixels.linkedin
						.pixel_checkout_start;

				if (
					! checkoutStartPixel ||
					! checkoutStartPixel.startsWith( '<img' )
				) {
					return;
				}

				console.log(
					`PMC Piano tp.push [LinkedIn]: [addHandler=${ PIANO_EVENT_CHECKOUT_START }]`
				);
		...
		window.tp.push( [
			...
			function( conversion ) {
				console.log(
					`PMC Piano tp.push [LinkedIn]: [addHandler=${ PIANO_EVENT_CHECKOUT_COMPLETE }]`,
					conversion
				);

				const checkoutCompletePixel =
					window.pmcPianoData.trackingPixels &&
					window.pmcPianoData.trackingPixels.linkedin &&
					window.pmcPianoData.trackingPixels.linkedin
						.pixel_checkout_complete;

				if (
					! checkoutCompletePixel ||
					! checkoutCompletePixel.startsWith( '<img' )
				) {
					return;
				}
		...
		window.tp.push( [
			...
			function() {
				try {
					...
					console.log(
						`PMC Piano tp.push [digioh]: [addHandler=${ PIANO_EVENT_CHECKOUT_START }]`
					);
