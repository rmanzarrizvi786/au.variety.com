...
const cxpmc = {
	...
	init: function() {
		if (window.pmcCxenseData.compatMode === 'enable') {
			cX.options = { compat: { c1x: { wait: 3000 } } };
		}
	...
	addHeadJs: function( script ) {
		...
		scriptElement.onload = script.onloadCallback
			? script.onloadCallback
			: '';
		scriptElement.onerror = script.onErrorCallback
			? script.onErrorCallback
			: '';
