<?php
/**
 * Handles mce asset loading based on the tinyMCE version.
 *
 */

namespace MCE_Table_Buttons;
...
class MCE_Table_Buttons {
	...
	public static function mce_external_plugins( $plugin_array ) {
		...
		if ( version_compare( $tinymce_version, '400', '<' ) ) {
			...
			wp_register_style( 'mce-table-buttons', untrailingslashit( MCE_TABLE_BUTTONS_URL ) . '/tinymce3-assets/mce-table-buttons.css' );
			wp_print_styles( 'mce-table-buttons' );

			$plugin_array['table']           = untrailingslashit( MCE_TABLE_BUTTONS_URL ) . '/tinymce3-table/editor_plugin.js';
			$plugin_array['mcetablebuttons'] = untrailingslashit( MCE_TABLE_BUTTONS_URL ) . '/tinymce3-assets/mce-table-buttons.js';

		} elseif ( version_compare( $tinymce_version, '4100', '<' ) ) {

			$plugin_array['table'] = untrailingslashit( MCE_TABLE_BUTTONS_URL ) . '/tinymce4-table/plugin' . $variant . '.js';

		} else {

			$plugin_array['table'] = untrailingslashit( MCE_TABLE_BUTTONS_URL ) . '/tinymce41-table/plugin' . $variant . '.js';
	...
	public static function mce_buttons_2( $buttons ) {
		...
		if ( version_compare( $tinymce_version, '400', '<' ) ) {
			...
			$pos = array_search( 'undo', (array) $buttons, true );
			if ( ! $pos ) {
