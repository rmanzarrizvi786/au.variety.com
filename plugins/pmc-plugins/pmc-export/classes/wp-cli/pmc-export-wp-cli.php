...
namespace PMC\Export\WP_CLI;

use PMC\Export\Posts;

/**
 * CLI Commands for Editorial Reports.
 *
 */
class PMC_Export extends \PMC_WP_CLI_Base {
	/**
	 ...
	 *    $ wp pmc-export generate-metadata
	 *
	...
	public function generate_metadata( $args = array(), $assoc_args = array() ) {
		...
		$supported_post_types = \PMC\Export\Posts::get_instance()->get_supported_post_types();
			...
			do {
				...
				foreach ( $posts as $post_id ) {
					...
					if ( in_array( 'word_count', (array) $meta_fields, true ) ) {
						...
						// Get the word count.
						$word_count = \PMC\Export\Helper::get_post_word_count( $post_id );

						if ( ! $this->dry_run ) {

							// Store the number of images in post meta.
							$existing_word_count = get_post_meta( $post_id, \PMC\Export\Posts::WORD_COUNT_META_SLUG, true );

							if ( (int) $word_count !== (int) $existing_word_count ) {
								update_post_meta( $post_id, \PMC\Export\Posts::WORD_COUNT_META_SLUG, (int) $word_count );
							}
						}
					...
					if ( in_array( 'image_count', (array) $meta_fields, true ) ) {
						...
						$image_count = \PMC\Export\Helper::get_image_count( $post_id );

						if ( ! $this->dry_run ) {
							// Store the number of images in post meta.

							$existing_image_count = get_post_meta( $post_id, \PMC\Export\Posts::IMAGE_COUNT_META_SLUG, true );

							if ( (int) $image_count !== (int) $existing_image_count ) {
								update_post_meta( $post_id, \PMC\Export\Posts::IMAGE_COUNT_META_SLUG, (int) $image_count );
							}
						}
					...
					if ( in_array( 'categorization', (array) $meta_fields, true ) ) {
						...
						$categories = implode( ', ', \PMC\Export\Helper::get_post_taxonomy_categorization( $post_id, 'category' ) );
						$verticals  = implode( ', ', \PMC\Export\Helper::get_post_taxonomy_categorization( $post_id, 'vertical' ) );
						...
						if ( ! $this->dry_run ) {
							...
							$existing_categorization = get_post_meta( $post_id, \PMC\Export\Posts::CATEGORIZATION_META_SLUG, true );
							...
							if ( $existing_categorization !== $updated_categorization ) {
								...
								update_post_meta(
									$post_id,
									\PMC\Export\Posts::CATEGORIZATION_META_SLUG,
					...
					if ( ! empty( $csv_report_file ) ) {
						...
						$linked_gallery = get_post_meta( $post_id, \PMC\Export\Posts::ATTACHED_GALLERY_META_SLUG, true );
	 ...
	 *
	 *    # Generate Metadata for all the brands and all the post types.
	 *    $ wp pmc-export generate_report --report-file=./editorial-report.csv
	 *
	 *    # Generate report for pmc-gallery post type
	 *    $ wp pmc-export generate_report --post-types=pmc-gallery --report-file=./editorial-report.csv
	 *
	 *    # Generate report for all post types for a specific date range
	 *    $ wp pmc-export generate_report --start-date=27/10/2015 --end-date=03/03/2020
	 *
	...
	public function generate_report( $args = array(), $assoc_args = array() ) {
		...
		if ( ! defined( 'WP_IMPORTING' ) ) {
			// Cant mock this scenario, no proper way to test the code.
			define( 'WP_IMPORTING', true ); // @codeCoverageIgnore
		}
		...
		foreach ( $post_types as $post_type ) {
			...
			do {
				...
				foreach ( $posts as $post_id ) {
					...
					// Get the word count.
					$word_count = get_post_meta( $post_id, \PMC\Export\Posts::WORD_COUNT_META_SLUG, true );
					$this->_write_log( sprintf( ' - Number of words: %d', $word_count ), 0 );

					// Get number of images in the post.
					$image_count = get_post_meta( $post_id, \PMC\Export\Posts::IMAGE_COUNT_META_SLUG, true );
					$this->_write_log( sprintf( ' - Number of images %d', $image_count ), 0 );

					// Get post categorization.
					$categorization = json_decode( get_post_meta( $post_id, \PMC\Export\Posts::CATEGORIZATION_META_SLUG, true ) );
...
// Registering the CLI command, no need to test the code.
\WP_CLI::add_command( 'pmc-export', 'PMC\Export\WP_CLI\PMC_Export' ); // @codeCoverageIgnore
