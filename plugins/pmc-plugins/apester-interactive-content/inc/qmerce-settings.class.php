...
class Qmerce_Settings
{
    ...
    public function postTypesCb()
    {
        ...
        foreach($postTypes as $postType) {
            ...
            printf(
                '<input type="checkbox" name="qmerce-settings-admin[post_types][]" value="%s" ' . esc_attr( $checked ) . '/> ' . esc_html( $postType->label ) . ' ',
    ...
    public function automationPostTypeCb()
    {
        ...
        foreach($postTypes as $postType) {
            ...
            printf(
                '<input type="checkbox" name="qmerce-settings-admin[automation_post_types][]" value="%s" ' . esc_attr( $checked ) . '/> ' . esc_html( $postType->label ) . ' ',
                esc_attr( $postType->name )
            );
        }
    }
    ...
    {
        printf(
            'Get a token at <a href="' . esc_url( APESTER_EDITOR_BASEURL.'/register' ) . '" target=_blank>Apester.com</a> (you can find it in your user <a href="' . esc_url( APESTER_EDITOR_BASEURL.'/user/settings/' ). '" target=_blank>settings</a>.)'
        );
    }
