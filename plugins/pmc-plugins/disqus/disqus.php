...
function dsq_output_loop_comment_js($post_ids = null) {
	...
	$_HAS_COUNTS = true;
	$render_disqus_script = apply_filters( 'pmc_render_disqus_script', true );

	if ( count( $post_ids  ) &&  $render_disqus_script ) {
?>
	<script type="text/javascript">
	// <![CDATA[
...
function dsq_output_footer_comment_js() {
	$render_disqus_script = apply_filters( 'pmc_render_disqus_script', true );
	if ( get_option('disqus_cc_fix') == '1' && $render_disqus_script ) {
?>
	<script type="text/javascript">
	// <![CDATA[
		var disqus_shortname = <?php echo wp_json_encode( strtolower(get_option('disqus_forum_url'))) ?>;
