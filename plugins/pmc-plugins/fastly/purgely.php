...
class Purgely
{
    ...
    public function __construct()
    {
        ...
        self::$surrogate_keys_header = new Purgely_Surrogate_Keys_Header();

        // Initialize cache control header.
        self::$cache_control_headers = new Purgely_Cache_Control_Header();

        // Initialize the surrogate control header.
        self::$surrogate_control_header = new Purgely_Surrogate_Control_Header();
    ...
    public function set_standard_keys()
    {
        ...
        self::$surrogate_keys_collection = $key_collection;

        $keys = $key_collection->get_keys();
        self::$surrogate_keys_header->add_keys($keys);
    }
    ...
    public function send_surrogate_keys()
    {
        ...
        $keys_header = self::$surrogate_keys_header;
    ...
    public function send_surrogate_control()
    {
        ...
        $surrogate_control = self::$surrogate_control_header;

        $custom_ttl = self::$surrogate_keys_collection->get_custom_ttl();
        if($custom_ttl) {
    ...
    public function send_cache_control()
    {
        ...
        $cache_control = self::$cache_control_headers;
            ...
            if(is_array($custom_post_types) && !empty($custom_post_types)) {
                foreach ($custom_post_types  as $post_type) {
                    ...
                    if(!$result && Purgely_Settings::get_setting('fastly_debug_mode')) {
						// Ignoring this error logging since it is dependent on debug option
						// being enabled and it will not log otherwise
                        error_log('Error when registering Fastly cache tags to custom post type: ' . $post_type);    // phpcs:ignore
                    }
                }
            }
        }
