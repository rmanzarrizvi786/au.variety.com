...
class Purgely_Surrogate_Key_Collection
{
    ...
    public function __construct($wp_query)
    {
        ...
        // We use mixed type return result here to use boolean to indicate no override
        // Where we allow any array, including empty array to indicate override values had been done
        // and bypass any addition keys generation
        $term_keys = apply_filters( 'purgely_pre_term_keys', false, $wp_query );
        ...
        if ( ! is_array( $term_keys ) ) {
            ...
            if ( $wp_query->is_single() ) {
                ...
                foreach ( $taxonomies as $taxonomy ) {
                    $term_keys = array_merge( (array) $term_keys, (array) $this->_add_key_terms_single( $wp_query->post->ID, $taxonomy ) );
                }

                // Get author information.
                $term_keys = array_merge( (array) $term_keys, (array) $this->_add_key_author( $wp_query->post ) );
            } else {
        ...
        foreach($always_purged as $k) {
            if (in_array($k, $template_key, true)) {
                $keys = $template_key;
                break;
            }
        }
