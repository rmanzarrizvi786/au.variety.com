    ...
    try {
        ...
        if (!$response->success) {
            if (Purgely_Settings::get_setting('fastly_debug_mode')) {
                ...
                error_log("Webhooks request failed, error: " . json_decode($response->body));    // phpcs:ignore
            }
        }
    } catch (Exception $e) {
    }
...
function test_web_hook()
{
    ...
    try {
        ...
        if (Purgely_Settings::get_setting('fastly_debug_mode')) {
			// Ignoring this error logging since it is dependent on debug option
			// being enabled and it will not log otherwise
            error_log('Webhooks - test connection: ' . $response->body);    // phpcs:ignore
        }
        ...
        if (Purgely_Settings::get_setting('fastly_debug_mode')) {
			// Ignoring this error logging since it is dependent on debug option
			// being enabled and it will not log otherwise
            error_log('Webhooks - test connection: ' . $e->getMessage());    // phpcs:ignore
        }
