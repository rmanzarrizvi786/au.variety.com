...
class Purgely_Purges
{
    ...
    public function __construct()
    {
        foreach ($this->_purge_actions() as $action) {
            // Need to be the last priority after all save action have been fired and data saved to db
            add_action($action, array($this, 'purge'), 999, 1);
        }
    }
    ...
    public function purge($post_id)
    {
        ...
        if (!in_array(get_post_status($post_id), array('publish', 'trash', 'future'), true)) {
            return;
        }
