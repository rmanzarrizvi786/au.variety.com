...
if ( !class_exists( 'O2O' ) ) {
	pmc_load_plugin( 'objects-to-objects', 'pmc-plugins' ); //VIP deprecated plugin now in our pmc-plugins repo
}

if ( !class_exists( 'MultiplePostAttachments' ) ) {
	pmc_load_plugin( 'multiple-post-attachments', 'pmc-plugins' );
}
...
class PMC_Publication_Issue {
	...
	public function action_admin_enqueue_scripts() {
		wp_enqueue_style( self::POST_TYPE .'wp-admin-css', plugins_url( 'css/wp-admin.css', __FILE__ ) );
	}

	/**
	 * We need to hook into the wp hook so we can re-assign the main query if needed
	 */
	public function action_wp( $wp ) {
		...
		// we need to check to make sure the queried post type is a custom publication issue before proceeding
		if ( !empty( $queried_object->post_type ) ) {
			if ( self::POST_TYPE == $queried_object->post_type ) {
				// we need to track the queried object for later use
				$this->issue = $queried_object;
				// treat publication issue as archive?
				if ( apply_filters('pmc-publication-issue-as-archive', true ) ) {
					$args = array();
					// pagination?
					if ( $paged = get_query_var( 'paged' ) ) {
						$args['paged'] = $paged;
					}
					// switch main query to use new query filter
					query_posts( PMC_Publication_Issue::get_instance()->query_args( $args ) );
				}
			}
		}
	...
	public function filter_template_include( $template ) {
		...
		if ( !empty( $this->issue->post_type ) && self::POST_TYPE == $this->issue->post_type ) {
			$new_template = locate_template( array( 'single-'. self::POST_TYPE . '.php', self::POST_TYPE . '.php' ) );
	...
	public function action_init() {
		...
		$this->supports = apply_filters( 'pmc-publication-issue-supports',
				array( 'pdf-attachment', 'sub-title', 'lead-article' )
			);
		...
		register_post_type(
			...
			array(
				...
				'hierarchical'        => false,
				'supports'            => array( 'title', 'thumbnail' ),  // we only support 2 built in features for now
				'taxonomies'          => array( self::PUB_TAXONOMY ), // we'll assign it later
				'has_archive'         => true, // we do want archive in the future
				'rewrite'             => false,  // we don't want wp default rewrite rules
				'query_var'           => self::POST_TYPE_SLUG, // important that we have a unique slug for this custom post type
				'can_export'          => true,
			)
		);
		...
		O2O::Register_Connection(
			...
			array(
				...
				'rewrite'    => false,
				'from' => array(
						'limit' => 1, // a post can only be assigned to 1 publication
						'labels' => array(
				...
				'to' => array(
						'limit' => self::MAX_POSTS_PER_ISSUE, // limit number of post can assign to a single publication
						'sortable' => true, // allow custom sorting
						'labels' => array(
							'name' => 'Posts',
							'singular_name' => 'Post'
						)
		...
		if ( in_array( 'pdf-attachment', $this->supports ) ) {
			new MultiplePostAttachments( array(
		...
		if ( in_array( 'publication', $this->supports ) ) {
		...
		if ( in_array( 'lead-article', $this->supports ) ) {
	...
	public function filter_wp_insert_post_data( $data , $postarr ) {
		...
		if ( !in_array( 'auto-title', $this->supports ) || self::POST_TYPE != $data['post_type'] || !empty( $data['post_title'] ) ) {
			return $data;
		}
		...
		$data['post_title'] = trim( apply_filters( 'publication-issue-title', $publication. ' ' . mysql2date( 'Y-m-d', $data['post_date'] ), $data, $postarr ) );
	...
	public function action_save_post( $post_id ) {
		if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
				...
				|| ! isset( $_POST[ 'pmclinkcontent-post-value-lead_article' ] // post data should contains the value we need
					)
		) {
		...
		$data = json_decode( trim( stripcslashes( $_POST[ 'pmclinkcontent-post-value-lead_article' ] ) ) );
		...
		if ( in_array( 'publication', $this->supports ) && taxonomy_exists( self::PUB_TAXONOMY ) ) {
	...
	public function action_add_meta_boxes() {
		...
		if ( in_array( 'lead-article', $this->supports ) ) {
			...
			add_meta_box(
				'pmc-publication-issue-lead-article', 'Lead Article', array( $this, 'meta_box_lead_article' ), self::POST_TYPE, 'normal', 'core'
			);
		}
	}
	...
	public function register_permastruct() {
		...
		if ( in_array( 'auto-title', $this->supports ) ) {
		...
		$perma_options = apply_filters( 'pmc-publication-issue-permastruct', $perma_options );
	...
	public function action_custom_metadata_manager_init_metadata() {
		...
		if ( taxonomy_exists( self::PUB_TAXONOMY ) ) {
			...
			$list = get_terms( self::PUB_TAXONOMY,
							array(
								'fields'     => 'names',
								'hide_empty' => false,
								'number'     => self::MAX_PUB_TERMS,  // IMPORTANT: to restrict to a safe number
							)
						);
			...
			$list = apply_filters( 'pmc-publication-list', $list );
			...
			if ( !empty( $list ) ) {
				...
				x_add_metadata_field(
						...
						array(
							...
							'label'       => 'Publication',
							'field_type'  => 'select',
							'values'      => $values, // values must be an associated array( key => value )
							)
					);
			}
		}
		...
		if ( in_array( 'sub-title', $this->supports ) ) {
			x_add_metadata_field( 'sub_title', array( self::POST_TYPE ),
				array(
					'group'       => self::POST_TYPE .'-grp',
					'label'       => 'sub-headline',
	...
	public function meta_box_lead_article( $post ) {
		...
		PMC_LinkContent::insert_field( json_encode( $data ), 'Article', 'lead_article' );
	} // function
	...
	/**
	 * Return articles associated to an issue, @see PMC_Publication_Issue::query_args
	 * @param int $issue_id The id of post of type pmc-pub-issue
	 * @return array The list of post object
	 */
	public function get_posts( $args = array() ) {
		$cache_key = md5( 'get-posts'. serialize( $args ) );
	...
	public function get_issues( $args = array() ) {
		$default = array(
				'posts_per_page'   => 1,
				'suppress_filters' => false,	// need this set for un-cached get_posts
			);
		$args = wp_parse_args( $args, $default );
		// make sure we query our custom post type, there is no overriding this value.
		$args['post_type'] = self::POST_TYPE;
		$cache_key = md5( 'get-issues'. serialize( $args ) );
	...
	public function post_to_issue( $post ) {
		...
		$args = array(
				'o2o_query' => array(
					'connection' => self::CONNECTION_NAME,
					'direction' => 'from', // we want issue => issue (from ) -> post(to)
					'id' => $post->ID,
				),
				'o2o_orderby' => self::CONNECTION_NAME,
				'posts_per_page' => 1
...
PMC_Publication_Issue::get_instance();
