...
final class AmpEmojiAttribute implements BeforeLoadFilter, AfterSaveFilter
{
    ...
    {
        $this->usedAmpEmoji = '';
        return \preg_replace_callback(self::AMP_EMOJI_ATTRIBUTE_PATTERN, function ($matches) {
    ...
    public function afterSave($html)
    {
        ...
        return \preg_replace('/(<html\\s[^>]*?)' . \preg_quote(Document::EMOJI_AMP_ATTRIBUTE_PLACEHOLDER, '/') . '="([^"]*)"/i', '\\1' . $this->usedAmpEmoji . '\\2', $html, 1);
    }
}
