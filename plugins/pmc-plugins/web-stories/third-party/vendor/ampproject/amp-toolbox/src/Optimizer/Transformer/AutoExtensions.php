...
use Google\Web_Stories_Dependencies\AmpProject\Dom\NodeWalker;
use Google\Web_Stories_Dependencies\AmpProject\Extension;
use Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\AutoExtensionsConfiguration;
use Google\Web_Stories_Dependencies\AmpProject\Optimizer\Error\CannotParseJsonData;
...
use Google\Web_Stories_Dependencies\AmpProject\Validator\Spec\AttributeList\GlobalAttrs;
...
final class AutoExtensions implements Transformer
{
     ...
     * @var array
     */
    const MANUAL_ATTRIBUTE_TO_EXTENSION_MAPPING = [Attribute::LIGHTBOX => Extension::LIGHTBOX_GALLERY];
    /**
     ...
     * @var array
     */
    private $protectedExtensions = ['amp-carousel' => ['amp-lightbox-gallery']];
    /**
        ...
        foreach ($document->head->getElementsByTagName(Tag::SCRIPT) as $script) {
            ...
            if ($script->getAttribute(Attribute::ID) === Extension::ACCESS) {
                ...
                foreach ($providers as $provider) {
                    ...
                    } elseif ($provider['vendor'] === 'scroll' && isset($provider['namespace']) && $provider['namespace'] === 'scroll') {
                ...
                foreach ($jsonData['services'] as $service) {
                    if (\array_key_exists('serviceId', $service) && $service['serviceId'] === 'subscribe.google.com') {
                        $extensionScripts = $this->maybeAddExtension($document, $extensionScripts, Extension::SUBSCRIPTIONS_GOOGLE);
                        break;
                    }
                }
            }
            $src = $script->getAttribute(Attribute::SRC);
            if (!$script->hasAttribute(Attribute::SRC) || Amp::CACHE_ROOT_URL !== \substr($src, 0, \strlen(Amp::CACHE_ROOT_URL))) {
                continue;
            }
    ...
    private function addRequiredExtensionByTag(Document $document, Element $node, $extensionScripts)
    {
        ...
        foreach ($tagSpecs as $tagSpec) {
            foreach ($tagSpec->requiresExtension as $requiredExtension) {
                if (\in_array($requiredExtension, self::MANUAL_ATTRIBUTE_TO_EXTENSION_MAPPING, \true)) {
                    continue;
                }
                $extensionScripts = $this->maybeAddExtension($document, $extensionScripts, $requiredExtension);
            }
    ...
    private function addExtensionForCustomTemplates(Document $document, Element $node, $extensionScripts)
    {
        ...
        return !$requiredExtension ? $extensionScripts : $this->maybeAddExtension($document, $extensionScripts, $requiredExtension);
    }
    /**
    ...
    private function addRequiredExtensionByAttributes(Document $document, Element $node, $extensionScripts)
    {
        ...
        foreach ($node->attributes as $attribute) {
            ...
            if (\array_key_exists($attribute->name, self::MANUAL_ATTRIBUTE_TO_EXTENSION_MAPPING)) {
                $extensionScripts = $this->maybeAddExtension($document, $extensionScripts, self::MANUAL_ATTRIBUTE_TO_EXTENSION_MAPPING[$attribute->name]);
            }
             ...
             */
            if ($this->configuration->get(AutoExtensionsConfiguration::EXPERIMENT_BIND_ATTRIBUTE) && \strpos($attribute->name, self::BIND_SHORT_FORM_PREFIX) === 0) {
                ...
                $bindableAttributeName = '[' . \str_replace('-', '_', \strtoupper($attributeNameWithoutBindPrefix)) . ']';
    ...
    private function removeUnneededExtensions($extensionScripts)
    {
        ...
        return \array_filter($extensionScripts, function ($extension) {
            return \array_key_exists($extension, $this->addedExtensions) || $this->isProtectedExtension($extension);
    ...
    private function renderExtensionScripts(Document $document, array $extensionScripts)
    {
        ...
        foreach ($extensionScripts as $extensionScript) {
            if ($referenceNode && $referenceNode->nextSibling) {
                $referenceNode->parentNode->insertBefore($extensionScript, $referenceNode->nextSibling);
            } else {
    ...
    private function getExtensionScriptsReferenceNode(Document $document)
    {
        ...
        while ($remainingNode) {
            ...
            if ($remainingNode->tagName === Tag::STYLE && $remainingNode->hasAttribute(Attribute::AMP_BOILERPLATE)) {
                $referenceNode = $remainingNode;
            } elseif ($remainingNode->tagName === Tag::NOSCRIPT && $remainingNode->firstChild instanceof Element && $remainingNode->firstChild->tagName === Tag::STYLE && $remainingNode->firstChild->hasAttribute(Attribute::AMP_BOILERPLATE)) {
                $referenceNode = $remainingNode;
            }
            $remainingNode = $remainingNode->nextSibling;
        }
    ...
    {
        if (\in_array($requiredExtension, $this->configuration->get(AutoExtensionsConfiguration::IGNORED_EXTENSIONS), \true)) {
            return $extensionScripts;
        }
        ...
        $this->addedExtensions[$requiredExtension] = \true;
        return $extensionScripts;
    }
    /**
    ...
    {
        try {
            $jsonString = \trim($script->nodeValue);
            if (empty($jsonString)) {
                return [];
            }
            $jsonData = \json_decode(\trim($script->nodeValue), \true);
