...
final class HttpEquivCharset implements BeforeLoadFilter, AfterLoadFilter, BeforeSaveFilter, AfterSaveFilter
{
    ...
    public function beforeLoad($html)
    {
        ...
        $html = \preg_replace(self::HTML_GET_HEAD_OPENING_TAG_PATTERN, self::HTML_GET_HEAD_OPENING_TAG_REPLACEMENT, $html, 1, $count);
        // If no <head> was found, we look for the <html> tag instead.
        if ($count < 1) {
            $html = \preg_replace(self::HTML_GET_HTML_OPENING_TAG_PATTERN, self::HTML_GET_HTML_OPENING_TAG_REPLACEMENT, $html, 1, $count);
        }
