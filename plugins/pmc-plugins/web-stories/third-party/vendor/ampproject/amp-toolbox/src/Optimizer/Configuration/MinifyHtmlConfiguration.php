...
final class MinifyHtmlConfiguration extends BaseTransformerConfiguration
{
    ...
    {
        return [self::MINIFY => \true, self::MINIFY_AMP_SCRIPT => \false, self::MINIFY_JSON => \true, self::COLLAPSE_WHITESPACE => \true, self::REMOVE_COMMENTS => \true, self::COMMENT_IGNORE_PATTERN => ''];
    }
    /**
    ...
    protected function validate($key, $value)
    {
        switch ($key) {
            ...
            case self::REMOVE_COMMENTS:
                if (!\is_bool($value)) {
                    throw InvalidConfigurationValue::forInvalidSubValueType(self::class, $key, 'boolean', \is_object($value) ? \get_class($value) : \gettype($value));
                }
                break;
            case self::COMMENT_IGNORE_PATTERN:
                if (!\is_string($value)) {
                    throw InvalidConfigurationValue::forInvalidSubValueType(self::class, self::COMMENT_IGNORE_PATTERN, 'string', \is_object($value) ? \get_class($value) : \gettype($value));
                }
                $value = \trim($value);
                break;
        }
