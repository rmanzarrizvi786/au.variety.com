...
use Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\MinifyHtmlConfiguration;
...
use DOMText;
use Exception;
use Google\Web_Stories_Dependencies\Peast\Formatter\Compact;
use Google\Web_Stories_Dependencies\Peast\Peast;
use Google\Web_Stories_Dependencies\Peast\Renderer;
/**
...
final class MinifyHtml implements Transformer
{
     ...
     */
    private function minifyNode(DOMNode $node, ErrorCollection $errors, $canCollapseWhitespace = \true, $inBody = \false)
    {
        $nodesToRemove = [];
        ...
        if ($node->hasChildNodes()) {
            ...
            foreach ($node->childNodes as $childNode) {
                $nodesToRemove = \array_merge($nodesToRemove, $this->minifyNode($childNode, $errors, $canCollapseWhitespace, $inBody));
            }
            if ($isNodeMustacheTemplate) {
                $this->mustacheTemplateDepth--;
            }
     ...
     * @return DOMNode[] Array of nodes to be removed.
     */
    private function minifyTextNode(DOMText $node, $canCollapseWhitespace = \true, $inBody = \false)
    {
    ...
    {
        return Tag::SCRIPT !== $tagName && Tag::STYLE !== $tagName && Tag::PRE !== $tagName && Tag::TEXTAREA !== $tagName;
    }
    /**
    ...
    private function minifyAmpScript(DOMText $node, ErrorCollection $errors)
    {
        if (!\class_exists(Peast::class) || !\class_exists(Renderer::class) || !\class_exists(Compact::class)) {
            $errors->add(Error\MissingPackage::withMessage('The optional package mck89/peast is required to minify inline amp-script.'));
            return;
        }
        ...
        try {
            ...
            $errors->add(Error\CannotMinifyAmpScript::withMessage($node->data, $error->getMessage()));
        }
    ...
    {
        if ($node instanceof Element && $node->tagName === Tag::TEMPLATE && $node->getAttribute(Attribute::TYPE) === Extension::MUSTACHE) {
            return \true;
        }
        return \false;
    }
