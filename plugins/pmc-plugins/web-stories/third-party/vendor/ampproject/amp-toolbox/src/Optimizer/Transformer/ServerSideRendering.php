...
final class ServerSideRendering implements Transformer
{
     ...
     * @var string[]
     */
    const SUPPORTED_LAYOUTS = ['', Layout::NODISPLAY, Layout::FIXED, Layout::FIXED_HEIGHT, Layout::RESPONSIVE, Layout::CONTAINER, Layout::FILL, Layout::FLEX_ITEM, Layout::FLUID, Layout::INTRINSIC];
    /**
    ...
    public function transform(Document $document, ErrorCollection $errors)
    {
        ...
        foreach ($document->xpath->query('.//script[ @custom-element ]', $document->head) as $customElementScript) {
            ...
            if ($customElementScript->getAttribute(Attribute::CUSTOM_ELEMENT) !== Extension::EXPERIMENT && Amp::isRenderDelayingExtension($customElementScript)) {
    ...
    private function applyLayout(Document $document, Element $element, ErrorCollection $errors)
    {
        ...
        try {
            ...
            $errors->add(Error\CannotPerformServerSideRendering::fromMaxCssByteCountExceededException($exception, $element));
            return \false;
        }
        return $newElement;
    }
     ...
     */
    private function calculateLayout($inputLayout, CssLength $width, CssLength $height, $sizesAttr, $heightsAttr)
    {
    ...
    {
        return \in_array($layout, Layout::SIZE_DEFINED_LAYOUTS, \true);
    }
    /**
     ...
     */
    private function maybeAddSizerInto(Document $document, Element $element, $layout, CssLength $width, CssLength $height)
    {
     ...
     * @return Element
     */
    private function createResponsiveSizer(Document $document, CssLength $width, CssLength $height, $style = 'padding-top:%s%%')
    {
    ...
    private function createIntrinsicSizer(Document $document, CssLength $width, CssLength $height)
    {
        ...
        $sizer_img->setAttribute(Attribute::SRC, \sprintf('data:image/svg+xml;base64,%s', \base64_encode("<svg height=\"{$height->getNumeral()}\" width=\"{$width->getNumeral()}\" " . "xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\"/>")));
    ...
    private function isAmpExperimentUsed(Element $element)
    {
        ...
        while ($child) {
            if ($child instanceof Element && $child->tagName === Tag::SCRIPT && \strtolower($child->getAttribute(Attribute::TYPE)) === Attribute::TYPE_JSON) {
                $script = $child;
                break;
            }
     ...
     */
    private function extractAttributeCss(Document $document, Element $element, DOMAttr $attribute, $mainStyle, $mediaQueryStyle)
    {
        if (empty($attribute->nodeValue)) {
            return [];
        }
        $sourceSizes = \explode(',', $attribute->nodeValue);
        $lastItem = \trim(\array_pop($sourceSizes), CssRule::CSS_TRIM_CHARACTERS);
        if (empty($lastItem)) {
        ...
        foreach (\array_reverse($sourceSizes) as $sourceSize) {
            ...
            $mediaCondition = \trim($matches['media_condition'], CssRule::CSS_TRIM_CHARACTERS);
            if (empty($mediaCondition)) {
                throw InvalidHtmlAttribute::fromAttribute($attribute->nodeName, $element);
            }
            $dimension = \trim($matches['dimension'], CssRule::CSS_TRIM_CHARACTERS);
            if (empty($dimension)) {
                throw InvalidHtmlAttribute::fromAttribute($attribute->nodeName, $element);
            }
            $cssRules[] = CssRule::withMediaQuery(\sprintf($mediaQueryStyle[0], $mediaCondition), $mediaQueryStyle[1], \sprintf($mediaQueryStyle[2], $dimension));
        }
        $elementId = $document->getElementId($element);
        $cssRules = \array_map(static function (CssRule $cssRule) use($elementId) {
    ...
    {
        $attributeValue = \trim($attribute->nodeValue, CssRule::CSS_TRIM_CHARACTERS);
        ...
        return [CssRule::withMediaQuery("@media {$not}{$attributeValue}", '#__ID__', 'display:none')->applyID($document->getElementId($element))];
    }
    /**
