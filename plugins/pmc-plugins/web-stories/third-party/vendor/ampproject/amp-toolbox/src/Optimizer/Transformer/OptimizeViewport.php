...
use Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\OptimizeViewportConfiguration;
            ...
            foreach ($metaTags as $metaTag) {
                ...
                foreach ($propertyPairs as $propertyPair) {
                    ...
                    if (isset($explodedPair[1])) {
                        $parsedRules[\trim($explodedPair[0])] = \trim($explodedPair[1]);
                    }
                }
                $metaTag->parentNode->removeChild($metaTag);
            }
            // Remove initial-scale=1 to leave just width=device-width in order to avoid a tap delay that hurts FID.
            if ($this->configuration->get(OptimizeViewportConfiguration::REMOVE_INITIAL_SCALE_VIEWPORT_PROPERTY) && isset($parsedRules[self::INITIAL_SCALE]) && \abs((float) $parsedRules[self::INITIAL_SCALE] - 1.0) < 0.0001) {
                unset($parsedRules[self::INITIAL_SCALE]);
            }
            $viewport = \implode(',', \array_map(static function ($ruleName) use($parsedRules) {
