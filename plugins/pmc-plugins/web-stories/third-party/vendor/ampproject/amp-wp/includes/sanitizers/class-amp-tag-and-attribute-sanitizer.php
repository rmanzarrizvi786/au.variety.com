...
class AMP_Tag_And_Attribute_Sanitizer extends AMP_Base_Sanitizer
{
    ...
    public function __construct($dom, $args = [])
    {
        ...
        $this->DEFAULT_ARGS = ['amp_allowed_tags' => AMP_Allowed_Tags_Generated::get_allowed_tags(), 'amp_globally_allowed_attributes' => AMP_Allowed_Tags_Generated::get_allowed_attributes(), 'amp_layout_allowed_attributes' => AMP_Allowed_Tags_Generated::get_layout_attributes(), 'prefer_bento' => \false];
    ...
    private function get_rule_spec_list_to_validate(\DOMElement $node, $rule_spec)
    {
        ...
        if (isset($rule_spec[AMP_Rule_Spec::TAG_SPEC]['extension_spec'])) {
            ...
            $rule_spec[AMP_Rule_Spec::ATTR_SPEC_LIST][$custom_attr] = [AMP_Rule_Spec::VALUE => $extension_spec['name'], AMP_Rule_Spec::MANDATORY => \true];
            $rule_spec[AMP_Rule_Spec::ATTR_SPEC_LIST]['src'] = [AMP_Rule_Spec::VALUE_REGEX => \implode('', ['^', \preg_quote('https://cdn.ampproject.org/v0/' . $extension_spec['name'] . '-', '/'), '(' . \implode('|', \array_merge($extension_spec['version'], ['latest'])) . ')', '\\.js$'])];
        }
    ...
    private function process_node(\DOMElement $node)
    {
        ...
        foreach ($rule_spec_list as $id => $rule_spec) {
            ...
            if (\true === $validity) {
                ...
                $validation_errors[] = \array_merge($validity, ['spec_name' => $this->get_spec_name($node, $rule_spec[AMP_Rule_Spec::TAG_SPEC])]);
            }
        }
        ...
        if (1 === \count($rule_spec_list_to_validate)) {
            ...
            if (1 === \count($spec_ids_sorted)) {
                ...
                foreach ($spec_ids_sorted as $id) {
                    ...
                    $tag_spec = \array_merge($tag_spec, $rule_spec_list_to_validate[$id][AMP_Rule_Spec::TAG_SPEC]);
        ...
        if (!empty($tag_spec[AMP_Rule_Spec::MANDATORY_ANYOF])) {
            ...
            if (0 === \count($anyof_attributes)) {
                $sanitized = $this->remove_invalid_child($node, [
                    ...
                    'mandatory_anyof_attrs' => $tag_spec[AMP_Rule_Spec::MANDATORY_ANYOF],
        ...
        if (!empty($tag_spec[AMP_Rule_Spec::MANDATORY_ONEOF])) {
            ...
            if (0 === \count($oneof_attributes)) {
                $sanitized = $this->remove_invalid_child($node, [
                    ...
                    'mandatory_oneof_attrs' => $tag_spec[AMP_Rule_Spec::MANDATORY_ONEOF],
                    // @todo Temporary as value can be looked up via spec name. See https://github.com/ampproject/amp-wp/pull/3817.
                    'spec_name' => $this->get_spec_name($node, $tag_spec),
                ]);
                return $sanitized ? null : $this->get_required_script_components($node, $tag_spec, $attr_spec_list);
    ...
    private function get_required_script_components(\DOMElement $node, $tag_spec, $attr_spec_list)
    {
        ...
        if ($node instanceof \DOMElement && !\in_array('amp-bind', $this->script_components, \true)) {
    ...
    private function validate_tag_spec_for_node(\DOMElement $node, $tag_spec)
    {
        if (!empty($tag_spec[AMP_Rule_Spec::MANDATORY_PARENT]) && !$this->has_parent($node, $tag_spec[AMP_Rule_Spec::MANDATORY_PARENT])) {
            return ['code' => self::WRONG_PARENT_TAG, 'required_parent_name' => $tag_spec[AMP_Rule_Spec::MANDATORY_PARENT]];
        }
        ...
        if (!empty($tag_spec[AMP_Rule_Spec::MANDATORY_ANCESTOR]) && !$this->has_ancestor($node, $tag_spec[AMP_Rule_Spec::MANDATORY_ANCESTOR])) {
            return ['code' => self::MANDATORY_TAG_ANCESTOR, 'required_ancestor_name' => $tag_spec[AMP_Rule_Spec::MANDATORY_ANCESTOR]];
        }
    ...
    private function sanitize_disallowed_attribute_values_in_node(\DOMElement $node, $attr_spec_list)
    {
        ...
        foreach ($node->attributes as $attr_name => $attr_node) {
             ...
             */
            if (isset($attr_spec_rule[AMP_Rule_Spec::VALUE]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_CASEI]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_casei($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_CASEI, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_REGEX]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_regex($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_REGEX, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_REGEX_CASEI]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_value_regex_casei($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_ATTR_VALUE_REGEX_CASEI, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_URL][AMP_Rule_Spec::ALLOWED_PROTOCOL]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_allowed_protocol($node, $attr_name, $attr_spec_rule)) {
            ...
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_URL]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_valid_url($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::INVALID_URL, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_URL]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_empty($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::MISSING_URL, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_URL][AMP_Rule_Spec::ALLOW_RELATIVE]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_relative($node, $attr_name, $attr_spec_rule)) {
                $attrs_to_remove[] = [$attr_node, self::DISALLOWED_RELATIVE_URL, null];
            } elseif (isset($attr_spec_rule[AMP_Rule_Spec::DISALLOWED_VALUE_REGEX]) && AMP_Rule_Spec::FAIL === $this->check_attr_spec_rule_disallowed_value_regex($node, $attr_name, $attr_spec_rule)) {
    ...
    private function check_matching_attribute_value($attr_name, $attr_value, $spec_values)
    {
        ...
        foreach ($spec_values as $spec_value) {
            ...
            $is_bool_match = '' === $spec_value && \in_array($attr_name, AMP_Rule_Spec::$boolean_attributes, \true) && \strtolower($attr_value) === \strtolower($attr_name);
            if ($is_bool_match) {
                return \true;
            }
        }
        ...
        if (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_URL][AMP_Rule_Spec::ALLOWED_PROTOCOL])) {
            if ($node->hasAttribute($attr_name)) {
                foreach ($this->extract_attribute_urls($node->getAttributeNode($attr_name)) as $url) {
                    ...
                    if (isset($url_scheme) && !\in_array(\strtolower($url_scheme), $attr_spec_rule[AMP_Rule_Spec::VALUE_URL][AMP_Rule_Spec::ALLOWED_PROTOCOL], \true)) {
                        return AMP_Rule_Spec::FAIL;
                    }
                }
                return AMP_Rule_Spec::PASS;
    ...
    private function check_attr_spec_rule_value_properties(\DOMElement $node, $attr_name, $attr_spec_rule)
    {
        if (isset($attr_spec_rule[AMP_Rule_Spec::VALUE_PROPERTIES]) && $node->hasAttribute($attr_name)) {
            ...
            if (empty($property_errors)) {
                ...
                return [AMP_Rule_Spec::FAIL, $property_errors];
            }
        }
        return [AMP_Rule_Spec::NOT_APPLICABLE, []];
    }
