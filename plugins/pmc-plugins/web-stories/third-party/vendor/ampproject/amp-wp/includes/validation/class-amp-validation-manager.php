...
use Google\Web_Stories_Dependencies\AmpProject\AmpWP\Sandboxing;
...
class AMP_Validation_Manager
{
    ...
    public static function init()
    {
        ...
        add_action('activate_plugin', static function () {
            if (!has_action('shutdown', [__CLASS__, 'validate_after_plugin_activation']) && self::get_dev_tools_user_access()->is_user_enabled()) {
                add_action('shutdown', [__CLASS__, 'validate_after_plugin_activation']);
                // Shutdown so all plugins will have been activated.
            }
        });
        ...
        if (!amp_is_canonical()) {
            $filter_hooks = ['default_option_' . AMP_Options_Manager::OPTION_NAME, 'option_' . AMP_Options_Manager::OPTION_NAME];
    ...
    public static function post_supports_validation($post)
    {
        ...
        return \in_array($post->post_type, AMP_Post_Type_Support::get_eligible_post_types(), \true) && !wp_is_post_autosave($post) && !wp_is_post_revision($post) && 'auto-draft' !== $post->post_status && 'trash' !== $post->post_status && amp_is_post_supported($post);
    }
    /**
    ...
    {
        if ($error && amp_is_canonical() && !Sandboxing::is_needed()) {
    ...
    public static function add_admin_bar_menu_items($wp_admin_bar)
    {
        ...
        if (current_user_can('manage_options')) {
            $wp_admin_bar->add_node(['parent' => 'amp', 'id' => 'amp-settings', 'title' => esc_html__('Settings', 'amp'), 'href' => esc_url(admin_url(add_query_arg('page', AMP_Options_Manager::OPTION_NAME, 'admin.php')))]);
        }
    ...
    {
        $override_validation_error_statuses = isset($_REQUEST['preview']) && !empty($_REQUEST[AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY]) && \is_array($_REQUEST[AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY]);
         ...
         */
        foreach ($_REQUEST[AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY] as $slug => $data) {
    ...
    public static function filter_the_editor_to_detect_sources($output)
    {
        ...
        foreach ($backtrace as $call) {
            if (isset($call['function'], $call['file']) && 'wp_editor' === $call['function']) {
    ...
    public static function add_validation_error(array $error, array $data = [])
    {
        ...
        $sanitized = AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_NEW_ACCEPTED_STATUS === $sanitization['status'] || AMP_Validation_Error_Taxonomy::VALIDATION_ERROR_ACK_ACCEPTED_STATUS === $sanitization['status'];
        /*
    ...
    public static function locate_sources(\DOMNode $node)
    {
        ...
        $is_enqueued_link = $node instanceof \DOMElement && 'link' === $node->nodeName && \preg_match('/(?P<handle>.+)-css$/', (string) $node->getAttribute('id'), $matches) && wp_styles()->query($matches['handle']);
        if ($is_enqueued_link) {
        ...
        $is_inline_style = $node instanceof \DOMElement && 'style' === $node->nodeName && $node->firstChild instanceof \DOMText && $node->hasAttribute('id') && \preg_match('/^(?P<handle>.+)-inline-css$/', $node->getAttribute('id'), $matches) && wp_styles()->query($matches['handle']) && isset(self::$extra_style_sources[$matches['handle']]);
        if ($is_inline_style) {
    ...
    public static function wrap_block_callbacks($args)
    {
        ...
        $args['render_callback'] = self::wrapped_callback(['function' => $original_function, 'source' => $source, 'accepted_args' => 3]);
        return $args;
    }
    /**
    ...
    public static function wrap_hook_callbacks($hook)
    {
        ...
        foreach ($wp_filter[$hook]->callbacks as $priority => &$callbacks) {
            foreach ($callbacks as &$callback) {
                ...
                $wrapped_callback = self::wrapped_callback(\array_merge($callback, \compact('priority', 'source', 'indirect_sources')));
                if (1 === $passed_by_ref) {
    ...
    public static function send_validate_response($sanitization_results, $status_code, $last_error)
    {
        ...
        if ($args[self::VALIDATE_QUERY_VAR_CACHE]) {
            ...
            $validated_url_post_id = AMP_Validated_URL_Post_Type::store_validation_errors($validation_errors, amp_get_current_url(), $data);
    ...
    public static function maybe_send_cached_validate_response()
    {
        ...
        if (!$args[self::VALIDATE_QUERY_VAR_OMIT_STYLESHEETS]) {
            $stylesheets = get_post_meta($post->ID, AMP_Validated_URL_Post_Type::STYLESHEETS_POST_META_KEY, \true);
            if ($stylesheets) {
        ...
        $queried_object = get_post_meta($post->ID, AMP_Validated_URL_Post_Type::QUERIED_OBJECT_POST_META_KEY, \true);
        if ($queried_object) {
            $response['queried_object'] = $queried_object;
        }
        $php_fatal_error = get_post_meta($post->ID, AMP_Validated_URL_Post_Type::PHP_FATAL_ERROR_POST_META_KEY, \true);
        if ($php_fatal_error) {
    ...
    public static function filter_sanitizer_args($sanitizers)
    {
        ...
        if (isset($sanitizers[AMP_Style_Sanitizer::class])) {
            ...
            foreach (self::$validation_error_status_overrides as $slug => $status) {
                ...
                $is_css_validation_error = \is_array($validation_error) && isset($validation_error['code']) && \in_array($validation_error['code'], AMP_Style_Sanitizer::get_css_parser_validation_error_codes(), \true);
    ...
    public static function validate_after_plugin_activation()
    {
        ...
        if (\is_array($validity) && \count($validation_errors) > 0) {
            // @todo This should only warn when there are unaccepted validation errors.
    ...
    public static function validate_url_and_store($url, $post = null)
    {
        ...
        $result = AMP_Validated_URL_Post_Type::store_validation_errors(wp_list_pluck($validity['results'], 'error'), $validity['url'], $args);
    ...
    {
        global $pagenow;
        if ('plugins.php' === $pagenow && (!empty($_GET['activate']) || !empty($_GET['activate-multi']))) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            if (isset($invalid_plugins)) {
                ...
                $more_details_link = \sprintf('<a href="%s">%s</a>', esc_url(add_query_arg('post_type', AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, admin_url('edit.php'))), __('More details', 'amp'));
                \printf(
                    ...
                    esc_html(\sprintf(
                        ...
                        _n('Warning: The following plugin may be incompatible with AMP: %s.', 'Warning: The following plugins may be incompatible with AMP: %s.', \count($invalid_plugins), 'amp'),
    ...
    public static function enqueue_block_validation()
    {
        ...
        if (\function_exists('Google\\Web_Stories_Dependencies\\wp_set_script_translations')) {
            ...
            $locale_data = \function_exists('Google\\Web_Stories_Dependencies\\wp_get_jed_locale_data') ? wp_get_jed_locale_data('amp') : gutenberg_get_jed_locale_data('amp');
