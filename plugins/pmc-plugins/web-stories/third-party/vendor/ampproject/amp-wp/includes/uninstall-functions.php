...
function delete_posts()
{
    ...
    do {
        ...
        $post_ids = $wpdb->get_col($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_type = %s LIMIT %d OFFSET %d;", $post_type, $per_page, $offset));
...
function delete_terms()
{
    ...
    do {
        $offset = $per_page * $current_page;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cannot cache result since we're deleting the records.
        $term_ids = $wpdb->get_col($wpdb->prepare("SELECT term_id FROM {$wpdb->term_taxonomy} WHERE taxonomy = %s LIMIT %d OFFSET %d;", $taxonomy, $per_page, $offset));
