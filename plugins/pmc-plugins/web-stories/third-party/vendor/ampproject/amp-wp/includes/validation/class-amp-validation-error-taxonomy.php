...
class AMP_Validation_Error_Taxonomy
{
    ...
    public static function register()
    {
        ...
        register_taxonomy(self::TAXONOMY_SLUG, AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, [
            'labels' => [
            ...
            'meta_box_cb' => \false,
            'capabilities' => [
                'manage_terms' => AMP_Validation_Manager::VALIDATE_CAPABILITY,
                // Needed to give access to the term list table.
                'delete_terms' => AMP_Validation_Manager::VALIDATE_CAPABILITY,
    ...
    public static function filter_posts_where_for_validation_error_status($where, WP_Query $query)
    {
        ...
        if (!\in_array(AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, (array) $query->get('post_type'), \true)) {
            return $where;
        }
        $error_statuses = [];
    ...
    {
        if (self::TAXONOMY_SLUG !== $tax->name || !isset($_POST['post_type']) || AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== $_POST['post_type']) {
            return $url;
        }
    ...
    public static function render_link_to_invalid_urls_screen($taxonomy_name)
    {
        ...
        $id = 'link-errors-url';
        \printf('<a href="%s" class="page-title-action" id="%s" hidden style="margin-left: 1rem;">%s</a>', esc_url(add_query_arg('post_type', AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, admin_url('edit.php'))), esc_attr($id), esc_html__('View Validated URLs', 'amp'));
        ?>
		<script>
			jQuery( function( $ ) {
    ...
    public static function render_error_status_filter()
    {
        ...
        if ('edit-tags' === $screen_base) {
            ...
            $args = ['post_type' => AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, 'update_post_meta_cache' => \false, 'update_post_term_cache' => \false];
    ...
    {
        \printf('<option value="%s" %s>%s <span class="count">(%s)</span></option>', esc_attr($option_value), selected($selected_value, $option_value, \false), esc_html($option_text), esc_html(number_format_i18n($error_count)));
    }
    /**
    ...
    {
        if (!(self::TAXONOMY_SLUG === get_current_screen()->taxonomy || AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === get_current_screen()->post_type)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        ...
        if (!empty($_GET['amp_actioned']) && !empty($_GET['amp_actioned_count'])) {
            ...
            if ('delete' === $actioned) {
                $message = \sprintf(
                    ...
                    _n('Deleted %s instance of validation errors.', 'Deleted %s instances of validation errors.', number_format_i18n($count), 'amp'),
                    $count
                );
            }
            if ($message) {
    ...
    public static function filter_tag_row_actions($actions, WP_Term $tag)
    {
        ...
        if ('post.php' === $pagenow) {
            ...
            $actions['details'] = \sprintf('<a href="%s">%s</a>', admin_url(add_query_arg([self::TAXONOMY_SLUG => $term->name, 'post_type' => AMP_Validated_URL_Post_Type::POST_TYPE_SLUG], 'edit.php')), esc_html__('Details', 'amp'));
            if (0 === $term->count) {
        ...
        switch ($column_name) {
            case 'error_code':
                if ('post.php' === $pagenow) {
                    ...
                    $content .= '<p>';
                    $content .= \sprintf('<a class="row-title" href="%s">%s', admin_url(add_query_arg([self::TAXONOMY_SLUG => $term->name, 'post_type' => AMP_Validated_URL_Post_Type::POST_TYPE_SLUG], 'edit.php')), wp_kses_post(self::get_error_title_from_code($validation_error)));
                }
                if ('post.php' === $pagenow) {
                    $content .= '</button>';
                } else {
            ...
            case 'status':
                ...
                if ('post.php' === $pagenow) {
                    $status_select_name = \sprintf('%s[%s][%s]', AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY, $term->slug, AMP_Validation_Manager::VALIDATION_ERROR_TERM_STATUS_QUERY_VAR);
                    \ob_start();
                    ?>
            ...
            case 'sources_with_invalid_output':
                ...
                if (empty($error_summary[self::SOURCES_INVALID_OUTPUT])) {
                    ...
                    AMP_Validated_URL_Post_Type::render_sources_column($error_summary[self::SOURCES_INVALID_OUTPUT], $url_post_id);
                }
                break;
            case 'error_type':
            ...
            case 'reviewed':
                if ('post.php' === $pagenow) {
                    ...
                    $input_name = \sprintf('%s[%s][%s]', AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY, $term->slug, self::VALIDATION_ERROR_ACKNOWLEDGE_ACTION);
    ...
    public static function render_single_url_error_details($validation_error, $term, $wrap_with_details = \true, $with_summary = \true)
    {
        ...
        if (isset($_GET['post'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $validation_errors = AMP_Validated_URL_Post_Type::get_invalid_url_validation_errors((int) $_GET['post']);
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        ?></code></dd>
			...
			<?php 
        if (AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_TAG === $validation_error['code'] && isset($validation_error['node_attributes'])) {
            ?>
				<dt><?php 
			...
			<?php 
        } elseif (AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_ATTR === $validation_error['code'] && isset($validation_error['element_attributes'])) {
            ?>
				<dt><?php 
            esc_html_e('Invalid markup', 'amp');
            ?></dt>
        ...
        foreach ($sources as $i => $source) {
            ...
            foreach (\array_keys($source_table_rows) as $j => $key) {
                ...
                if ('sources' === $key && \is_array($value)) {
                    ...
                    ?>
										<pre><?php 
                    echo esc_html(wp_json_encode($source, 128 | 64));
                    ?></pre>
									<?php 
                }
                ?>
    ...
    public static function handle_inline_edit_request()
    {
        ...
        if (isset($_GET['post_type']) && AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== $_GET['post_type']) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
    ...
    {
        if (!isset($_REQUEST['action']) || AMP_Validated_URL_Post_Type::POST_TYPE_SLUG !== get_post_type($post_id)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        $action = sanitize_key($_REQUEST['action']);
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $term_ids = isset($_POST['delete_tags']) ? \array_map('sanitize_key', $_POST['delete_tags']) : null;
        // phpcs:ignore WordPress.Security.NonceVerification.Missing
        $single_term_id = isset($_GET['term_id']) ? sanitize_key($_GET['term_id']) : null;
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    public static function handle_validation_error_update($redirect_to, $action, $term_ids)
    {
        ...
        if ('edit.php' === $pagenow && 'delete' === $action && 1 === $updated_count) {
            ...
            $redirect_to = add_query_arg(['amp_actioned' => $action, 'amp_actioned_count' => $term_ids_count], esc_url(get_admin_url(null, 'edit-tags.php?taxonomy=' . self::TAXONOMY_SLUG . '&post_type=' . AMP_Validated_URL_Post_Type::POST_TYPE_SLUG)));
        } else {
    ...
    public static function get_error_title_from_code($validation_error)
    {
        switch ($validation_error['code']) {
            ...
            case AMP_Tag_And_Attribute_Sanitizer::INCORRECT_NUM_CHILD_TAGS:
                ...
                ), '<code>' . esc_html($validation_error['node_name']) . '</code>', esc_html(number_format_i18n((int) $validation_error['required_child_count'])));
            case AMP_Tag_And_Attribute_Sanitizer::INCORRECT_MIN_NUM_CHILD_TAGS:
                ...
                ), '<code>' . esc_html($validation_error['node_name']) . '</code>', esc_html(number_format_i18n((int) $validation_error['required_min_child_count'])));
    ...
    public static function get_source_key_label($key, $validation_error)
    {
        switch ($key) {
            ...
            case 'meta_property_name':
                if (AMP_Tag_And_Attribute_Sanitizer::DISALLOWED_PROPERTY_IN_ATTR_VALUE === $validation_error['code'] || AMP_Tag_And_Attribute_Sanitizer::MISSING_REQUIRED_PROPERTY_VALUE === $validation_error['code']) {
