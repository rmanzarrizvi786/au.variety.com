...
class AMP_Style_Sanitizer extends AMP_Base_Sanitizer
{
        ...
        foreach ($this->dom->xpath->query('//*/@class') as $class_attribute) {
            ...
            foreach (\array_filter(\preg_split('/\\s+/', \trim($class_attribute->nodeValue))) as $class_name) {
                if (\substr($class_name, 0, $i_amphtml_prefix_length) === $i_amphtml_prefix) {
                    $error = ['code' => self::DISALLOWED_ATTR_CLASS_NAME, 'type' => AMP_Validation_Error_Taxonomy::HTML_ATTRIBUTE_ERROR_TYPE, 'class_name' => $class_name];
    ...
    private function get_stylesheet_priority(\DOMNode $node)
    {
        ...
        if ($node instanceof \DOMElement && 'link' === $node->tagName) {
            ...
            $plugin = null;
            if (\preg_match(\sprintf('#^(?:%s|%s)(?<plugin>[^/]+)#i', \preg_quote($remove_url_scheme(trailingslashit(\WP_PLUGIN_URL)), '#'), \preg_quote($remove_url_scheme(trailingslashit(\WPMU_PLUGIN_URL)), '#')), $schemeless_href, $matches)) {
    ...
    private function process_style_element(\DOMElement $element)
    {
        ...
        $this->pending_stylesheets[] = ['group' => $is_keyframes ? self::STYLE_AMP_KEYFRAMES_GROUP_INDEX : self::STYLE_AMP_CUSTOM_GROUP_INDEX, 'original_size' => (int) \strlen($stylesheet), 'final_size' => null, 'element' => $element, 'origin' => 'style_element', 'sources' => $this->current_sources, 'priority' => $this->get_stylesheet_priority($element), 'tokens' => $parsed['tokens'], 'hash' => $parsed['hash'], 'parse_time' => $parsed['parse_time'], 'shake_time' => null, 'cached' => $parsed['cached'], 'imported_font_urls' => $parsed['imported_font_urls'], 'important_count' => $parsed['important_count'], 'kept_error_count' => $parsed['kept_error_count']];
    ...
    private function process_link_element(\DOMElement $element)
    {
        ...
        if ($this->allowed_font_src_regex && \preg_match($this->allowed_font_src_regex, $normalized_url)) {
            ...
            if ($needs_preconnect_link) {
                $link = AMP_DOM_Utils::create_node($this->dom, 'link', ['rel' => 'preconnect', 'href' => 'https://fonts.gstatic.com/', 'crossorigin' => '']);
        ...
        if ($stylesheet instanceof WP_Error) {
            $this->remove_invalid_child($element, ['code' => self::STYLESHEET_FETCH_ERROR, 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'url' => $normalized_url, 'message' => $stylesheet->get_error_message()]);
            return;
        }
    ...
    private function get_parsed_stylesheet($stylesheet, $options = [])
    {
        ...
        $cache_key = null;
        $cached = \true;
        $cache_group = 'amp-parsed-stylesheet-v37';
        ...
        $cache_impacting_options = \array_merge(wp_array_slice_assoc($options, ['property_allowlist', 'property_denylist', 'stylesheet_url', 'allowed_at_rules']), wp_array_slice_assoc($this->args, ['should_locate_sources', 'parsed_cache_variant', 'dynamic_element_selectors', 'transform_important_qualifiers']), [
    ...
    private function splice_imported_stylesheet(Import $item, CSSList $css_list, $options)
    {
        ...
        if ($stylesheet instanceof WP_Error) {
            $error = ['code' => self::STYLESHEET_FETCH_ERROR, 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'url' => $import_stylesheet_url, 'message' => $stylesheet->get_error_message()];
    ...
    private function create_validated_css_document($stylesheet_string, $options)
    {
        ...
        try {
            ...
            $error = ['code' => self::CSS_SYNTAX_PARSE_ERROR, 'message' => $exception->getMessage(), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
            /*
    ...
    private function parse_stylesheet($stylesheet_string, $options = [])
    {
        ...
        return \array_merge(\compact('tokens', 'validation_results'), ['imported_font_urls' => $parsed_stylesheet['imported_font_urls'], 'hash' => \md5(wp_json_encode($tokens)), 'parse_time' => \microtime(\true) - $start_time, 'viewport_rules' => $parsed_stylesheet['viewport_rules'], 'important_count' => $parsed_stylesheet['important_count']]);
    }
    /**
        ...
        foreach ($css_list->getContents() as $css_item) {
            ...
            if ($css_item instanceof DeclarationBlock && empty($options['validate_keyframes'])) {
                ...
                if (!\in_array($css_item->atRuleName(), $options['allowed_at_rules'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_AT_RULE, 'at_rule' => $css_item->atRuleName(), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    $validation_results[] = \compact('error', 'sanitized');
                }
                if (!$sanitized) {
                ...
                $error = ['code' => self::CSS_SYNTAX_INVALID_DECLARATION, 'item' => \get_class($css_item), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                $validation_results[] = \compact('error', 'sanitized');
            }
            if ($sanitized) {
        ...
        if (!empty($options['property_allowlist'])) {
            ...
            foreach ($properties as $property) {
                ...
                if (!\in_array($vendorless_property_name, $options['property_allowlist'], \true)) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY, 'css_property_name' => $property->getRule(), 'css_property_value' => $property->getValue(), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    if ($sanitized) {
            ...
            foreach ($options['property_denylist'] as $illegal_property_name) {
                ...
                foreach ($properties as $property) {
                    $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY_NOLIST, 'css_property_name' => $property->getRule(), 'css_property_value' => (string) $property->getValue(), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                    $sanitized = $this->should_sanitize_validation_error($error);
                    if ($sanitized) {
                        $ruleset->removeRule($property->getRule());
                    }
    ...
    private function process_font_face_at_rule(AtRuleSet $ruleset, $options)
    {
        ...
        if (isset($properties[0])) {
            $font_family = \trim($properties[0]->getValue(), '"\'');
    ...
    private function process_css_keyframes(KeyFrame $css_list, $options)
    {
        ...
        foreach ($css_list->getContents() as $rules) {
            if (!$rules instanceof DeclarationBlock) {
                $error = ['code' => self::CSS_SYNTAX_INVALID_DECLARATION, 'item' => \get_class($rules), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                if ($sanitized) {
            ...
            if (!empty($options['property_allowlist'])) {
                ...
                foreach ($properties as $property) {
                    $vendorless_property_name = \preg_replace('/^-\\w+-/', '', $property->getRule());
                    if (!\in_array($vendorless_property_name, $options['property_allowlist'], \true)) {
                        $error = ['code' => self::CSS_SYNTAX_INVALID_PROPERTY, 'css_property_name' => $property->getRule(), 'css_property_value' => (string) $property->getValue(), 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => $options['spec_name']];
                        $sanitized = $this->should_sanitize_validation_error($error);
                        if ($sanitized) {
    ...
    private function transform_important_qualifiers(RuleSet $ruleset, CSSList $css_list, $options)
    {
        ...
        foreach ($properties as $property) {
            ...
            if (!$this->args['transform_important_qualifiers']) {
                ...
                $error = ['code' => self::CSS_SYNTAX_INVALID_IMPORTANT, 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'css_property_name' => $property->getRule(), 'css_property_value' => $property->getValue(), 'spec_name' => $options['spec_name']];
                $sanitized = $this->should_sanitize_validation_error($error);
                if ($sanitized) {
                    $property->setIsImportant(\false);
                } else {
    ...
    private function finalize_styles()
    {
        ...
        ], self::STYLE_AMP_KEYFRAMES_GROUP_INDEX => ['source_map_comment' => "\n\n/*# sourceURL=amp-keyframes.css */", 'cdata_spec' => $this->style_keyframes_cdata_spec, 'included_count' => 0, 'import_front_matter' => '', 'important_count' => 0, 'kept_error_count' => 0, 'is_excessive_size' => \false]];
        $imported_font_urls = [];
    ...
    private function ampify_ruleset_selectors($ruleset)
    {
        ...
        foreach ($ruleset->getSelectors() as $old_selector) {
            ...
            if (\preg_match('/(^|\\W)i-amphtml-/', $selector)) {
                $error = ['code' => self::CSS_DISALLOWED_SELECTOR, 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'css_selector' => $selector];
                $sanitized = $this->should_sanitize_validation_error($error);
    ...
    private function finalize_stylesheet_group($group, $group_config)
    {
        ...
        foreach ($pending_stylesheet_indices as $i) {
            ...
            if (!$this->args['allow_excessive_css'] && $is_stylesheet_excessive) {
                $validation_error = ['code' => self::STYLESHEET_TOO_LONG, 'type' => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE, 'spec_name' => self::STYLE_AMP_KEYFRAMES_GROUP_INDEX === $group ? self::STYLE_AMP_KEYFRAMES_SPEC_NAME : self::STYLE_AMP_CUSTOM_SPEC_NAME];
