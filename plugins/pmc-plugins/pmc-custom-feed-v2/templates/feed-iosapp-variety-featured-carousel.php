...
if ( empty( $image_size ) ) {
	$image_size = 'featured-carousel';
} else {
	$image_size = 'pmc_custom_image' . $image_size;
}

$data = null;
if( !empty( $_GET['fpid'] ) && is_numeric( $_GET['fpid'] ) ) {
	$post = get_post( intval( $_GET['fpid'] ) );
	if ( ! empty( $post ) ) {
		setup_postdata($post);
...
if ( empty( $data ) && class_exists( 'PMC_Carousel' ) ) {
	...
	$data = pmc_render_carousel( PMC_Carousel::modules_taxonomy_name, 'featured-carousel', $post_count, $image_size );
