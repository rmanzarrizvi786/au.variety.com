		...
		wp.media.pmc_gallery = {
			...
			frame: function () {
				...
				this._frame.on('open', function(){
					self.Analytic = new window.pmc.GoogleAnalyticEvents({el: '#pmc-gallery', mediaFrame: self._frame });
				});

				this._frame.on( 'edit-attachments:open', function( _this ) {
					// Validate image credit on initial Modal open.
					setTimeout( function() { self.validateImageCredit( _this.model ) }, 500 ); // eslint-disable-line
				} );
			...
			onChangeAttachment: function (model) {
				 ...
				 * and restore here.
				 * we will change data silenttly because data is already stored in database.
				 */
				if ( 'undefined' !== typeof model.changed.id || 'gallery_edit' !== mode ) {
					...
					// If File modified and not previousully added then, it should be add in gallery.
			...
			onChangeAttachmentFromMediaLibrary: function (model, event) {
				...
				// Validate image credit on media library image change.
				// Set Image credit in model attributes.
				if ( 'undefined' !== typeof model.attributes.compat.item ) {
