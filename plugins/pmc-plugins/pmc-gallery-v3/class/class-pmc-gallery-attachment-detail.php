...
class Detail {
	...
	public function register_post_types() {
		$labels = array(
			'name' => _x( 'Gallery Attachment', 'post type general name', 'pmc-gallery-v3' ),
			'singular_name' => _x( 'Gallery Attachment', 'post type singular name', 'pmc-gallery-v3' ),
			'add_new' => _x( 'Add New', 'Gallery Attachment', 'pmc-gallery-v3' ),
			'add_new_item' => __( 'Add New Gallery Attachment', 'pmc-gallery-v3' ),
			'edit_item' => __( 'Edit Gallery Attachment', 'pmc-gallery-v3' ),
			'new_item' => __( 'New Gallery Attachment', 'pmc-gallery-v3' ),
			'all_items' => __( 'All Gallery Attachments', 'pmc-gallery-v3' ),
			'view_item' => __( 'View Gallery Attachment', 'pmc-gallery-v3' ),
			'search_items' => __( 'Search Gallery Attachment', 'pmc-gallery-v3' ),
			'not_found' => __( 'No Gallery Attachment found', 'pmc-gallery-v3' ),
			'not_found_in_trash' => __( 'No Gallery Attachment found in the Trash', 'pmc-gallery-v3' ),
			'parent_item_colon' => '',
			'menu_name' => __( 'Gallery Attachment', 'pmc-gallery-v3' ),
		);
		$args = array(
			'labels' => $labels,
			'description' => __( 'Holds gallery specific attachment\'s data', 'pmc-gallery-v3' ),
			'menu_position' => 5,
	 ...
	 * @return ARRAY_A Attachmet's custom data stored in private post.
	 */
	 ...
	 * @param ARRAY_A $data	  Attachment's custom data.
	 * @param int	  $variant_id Optional.  private post id if it is available.
	...
	public function add_attachment_variant( $gallery_id, $data = array(), $variant_id = false ) {
		...
		foreach ( $data as $key => $value ) {
			...
			if ( ! empty( $value ) && in_array( $key, $allow_keys, true ) ) {
				if ( in_array( $key, $allow_html_in, true ) ) {
		...
		$post_array = array(
			...
			'post_type' => self::name,
			'post_parent' => absint( $data['id'] ), // Set attachment id as post_parent. it will be helpfull in getting no of gallery where attachment id used.
			'post_status' => 'publish',
	 ...
	 *
	 * @param type $data data to fetch word.
	 * @return array return array with unique word.
	 */
	public function get_unique_word( $data = array() ) {
		$keywords = array();
		$common_word_list = array(
			'he', 'she', 'they', 'there', 'their', 'we', 'it',
			'am', 'is', 'are', 'can', 'do', 'does', 'will', 'was', 'were', 'like', 'an', 'not',
			'the', 'of', 'to', 'and', 'or', 'so'
		);
		...
		foreach ( $keywords as $key => $word ) {
			...
			if ( in_array( $word, $common_word_list ) ) {
				unset( $keywords[ $key ] );
			}
		}
		return $keywords;
...
\PMC\Gallery\Attachment\Detail::get_instance();
