<div class="hp-panel">
	<div class="pmc-stocks-global-index-widget">
		<h3>
		...
		<div class="pmc-stocks-global-index-graph"></div>
	...
	S1536,628.7,1536,768z"></path></svg>
		...
		<?php endif; ?>
		<?php if ( ! empty( $widget_data[ 'logo' ] ) ) { ?>
			<?php
				$sanitized_logo = wp_kses( $widget_data[ 'logo' ], [
						'img' => [
							'src'    => [],
							'class'  => [],
							'width'  => [],
			...
			?>
			<div class="pmc-stocks-global-index-presented-by">
				<span><?php esc_html_e( 'presented by', 'pmc-plugins' ); ?></span>
				<?php if ( ! empty( $widget_data[ 'logo_url' ] ) ) { ?>
					<a href="<?php echo esc_url( $widget_data[ 'logo_url' ] ); ?>" target="_blank"><?php echo $sanitized_logo; ?></a>
				<?php } else { ?>
					<?php echo $sanitized_logo; ?>
				<?php } ?>
			</div>
		<?php } ?>
	</div>
	<script>
		...
		jQuery(function() {
			if ( 'undefined' !== typeof window.pmc && 'undefined' !== typeof window.pmc.stocks ) {
				pmc.stocks.build_global_index( <?php echo wp_json_encode( $data ); ?>, <?php echo wp_json_encode( $summary ); ?>, <?php echo wp_json_encode( $gainers ); ?>, <?php echo wp_json_encode( $decliners ); ?> );
			}
		});
	</script>
</div>
