...
class Asset_Chooser {
	...
	public function handle_ajax_image_sideload() {
		...
		// Url fixes for improper urls delivered from webdam
		// 1) they supply protocol-less urls which are not valid for ingestion
		// 2) when choosing a 1280px image webdam still sends the medium version
		$webdam_asset_url = str_replace( '//', webdam_get_site_protocol(), $webdam_asset_url );
