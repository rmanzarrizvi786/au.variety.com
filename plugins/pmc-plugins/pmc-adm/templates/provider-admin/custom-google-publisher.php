<?php
if ( empty( $provider ) || ! is_a( $provider, 'Google_Publisher_Provider' ) ) {
	return;
}
?>
<div class="adm-column-2">
	...
	foreach ( $provider->get_fields() as $field => $data ) :
		...
		$data            = array_merge( array( 'required' => true, 'validator' => '' ), $data );
		...
		<div class="<?php echo esc_attr( $div_css_class ) ?>">
			<label for="<?php echo esc_attr( $provider_id . '-' . $field ); ?>"
				   class="<?php echo esc_attr( $label_css_class ); ?>">
