...
class PMC_Ads_Contextual_Player_Ad {
	...
	protected function __construct() {
		...
		add_filter( 'the_content', [ $this, 'inject_contextual_player_at_bottom' ] );
	...
	/**
	 * Inject Contextual player markup at bottom of post content
	 *
	 * @param $post_content string
	 *
	 * @return string $post_content
	 */
	public function inject_contextual_player_at_bottom( $post_content ) {
		...
		if ( $this->_should_contextual_player_render() ) {
			...
			if (
				...
				&& is_array( $this->_ad )
				&& isset( $this->_ad['contextual-player-position'] )
				&& 'bottom' === $this->_ad['contextual-player-position']
			) {
	...
	public function get_contextual_player_ad_markup() {
		...
		$config          = [
			...
			'playlist_id'         => empty( $this->_ad['playlist-id'] ) ? '' : $this->_ad['playlist-id'],
			...
			'position'            => empty( $this->_ad['contextual-player-position'] ) ? '' : $this->_ad['contextual-player-position'],
		];
