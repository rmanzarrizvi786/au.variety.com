...
class Instant_Articles {
	...
	public function add_cheezcap_options( $cheezcap_options = array() ) {
		...
		$cheezcap_options[] = new CheezCapDropdownOption(
			__( 'Inject associated gallery link in instant articles', 'pmc-gallery-v4' ),
			__( 'If enabled, associated gallery link will inject in instant articles', 'pmc-gallery-v4' ),
			self::OPTION_NAME,
	...
	public function inject_associated_gallery_link( $content, $post_id ) {
		...
		$launch_gallery_html = sprintf(
			...
			esc_html__( 'Launch Gallery', 'pmc-gallery-v4' ),
