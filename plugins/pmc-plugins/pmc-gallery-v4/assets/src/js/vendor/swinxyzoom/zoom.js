;( function( $, undefined ) {
	...
	function Zoom( element, options ) {
		...
		s._blur = function( e ) {
			...
			if ( s.options[ 'controls' ] ) {
				s.ct.j.hide();
				s.ct.ol.j.stop().animate( { opacity: 0.0 }, {
		...
		s._scale      = [];
		s._scale[ 1 ] = function( e ) {
			scaleStartLevel = s.level;
			$( '#info' ).append( 'start[' + scaleStartLevel + '] ' );
		};
		s._scale[ 2 ] = function( e ) {
			$( '#info' ).append( 's ' );
			$( '#info' ).append( 's[' + Math.abs( ( scaleStartLevel * e.scale ) ) + ',' + e.position.x + '] ' );
			s.zoom( ( scaleStartLevel * e.scale ), e.position.x, e.position.y );
		};
		...
		var delayedLoad = function() {
			...
			if ( s.dp.tn.j.attr( 'src' ) != s.dp.tn.src ) {
				s.dp.tn.j.one( 'load', function() {
					...
					s.dp.tn.j.stop().animate( { opacity: 1.0 }, {
						queue: false, complete: function() {
			...
			var hndLoad = function( x, y ) {
				...
				s.dp.j.on( 'sxy-move sxy-hover', _hndMove = function( e ) {
					var p    = e.pointers[ 0 ];
					s.cursor = { lastX: p.x, lastY: p.y };
				} );
	...
	Zoom.prototype.tearUp = function( cb ) {
		...
		s.getNaturalSize( s.el.tn.j.attr( 'src' ), function( thumbDimensions ) {
			...
			s.el.j.wrap( tplWrapper.replace( /\{(\w+)\}/g, function( s, k ) {
				return filters[ k ] || s;
			} ) );
			s.element.hide();
			...
			s.rt.j.append( tplContent.replace( /\{(\w+)\}/g, function( s, k ) {
				return filters[ k ] || s;
			} ) );
			s.rt.bg = new E( s.rt.j.find( '.sxy-zoom-bg' ) );

			s.rt.j.on( 'dragstart', function( e ) {
				return false;
			} );
			...
			// View Finder
			s.vf = new E( s.dp.j.find( '.sxy-zoom-viewfinder' ).css( { display: 'none' } ), {
				w: 0,
				h: 0,
				l: 0,
				t: 0,
			...
			s.dp.j.on( 'sxy-scale', function( e ) {
				s._scale[ e.state ]( e );
			} );
