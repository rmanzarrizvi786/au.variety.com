...
class Boomerang_Provider extends PMC_Ad_Provider {
	...
	public function include_assets() {
		...
		add_action( 'pmc-tags-top', [ $this, 'add_adhesion_adunit' ] );
	}
	...
	public function wp_enqueue_scripts() {
		...
		wp_enqueue_script( 'pmc-async-adm-boomerang-client-js', sprintf( '%s/js/boomerang/build/index.js', PMC_ADM_URL ), [ 'jquery', 'pmc-hooks' ], PMC_ADM_VERSION, true );
	...
	public function prepare_boomerang_global_settings() {
		...
		if ( ! empty( $keywords ) && empty( $targeting_data['kw'] ) ) {
			$targeting_data['kw'] = $keywords;
		}
		...
		$data['enable_jwplayer_monetization'] = ( PMC::is_amp() ) ? false : true;
	...
	protected function _prepare_ad_data( $data ) {
		...
		$data['zone'] = apply_filters( 'pmc_adm_google_publisher_slot', sprintf( '/%s', $data['zone'] ), $data );
		...
		// wrap ad-width value inside []
		$data['ad-width']     = ( ! empty( $data['ad-width'] ) ) ? json_decode( sprintf( '[%s]', $data['ad-width'] ) ) : '';
		$data['is_lazy_load'] = ( isset( $data['is_lazy_load'] ) ) ? $data['is_lazy_load'] : false;

		$data = $this->_apply_cls_optimization( (array) $data );
		$data = $this->_apply_slot_rotate_class( (array) $data );

		/**
		 * Filter to modify ad unit data.
		 */
		$data = apply_filters( 'pmc_adm_prepare_boomerang_ad_data', (array) $data );
	...
	protected function _apply_cls_optimization( array $data ) : array {
		...
		$cls_option = apply_filters(
			'pmc_adm_optimize_cls',
			\PMC_Cheezcap::get_instance()->get_option( 'pmc_optimize_cls' ),
			$data
		);
		...
		if ( 'disable' !== $cls_option
			...
			if ( ! empty( $cls_width ) && ! empty( $cls_height ) && $can_optimized ) {
				...
				// check if we have ad-text style applied, if we  need to increase the height to additional 25px
	...
	/**
	 * Apply slot-rotate-[class] to slot data base on rules for auto ad refresh trigger
	 * @param array $data
	 * @return array
	 */
	protected function _apply_slot_rotate_class( array $data ) : array {

		// ROP-2518 - ad trigger for rail-bottom, adhesion
		$targeting_data = [
			'pos' => [
		...
		foreach ( $data['targeting_data'] as $item ) {
			if ( ! empty( $item['key'] )
				&& ! empty( $item['value'] )
				&& ! empty( $targeting_data[ $item['key'] ] )
				&& ! empty( $targeting_data[ $item['key'] ][ $item['value'] ] )
			) {
				$classes[] = sprintf( 'slot-rotate-%s', $targeting_data[ $item['key'] ][ $item['value'] ] );
			}
		}

		$data['css-slot-rotate'] = implode( ' ', array_unique( (array) $classes ) );
	...
	/**
	 * Render Adhesion adunit -aka Mobile sticky ad
	 */
	public function add_adhesion_adunit() {
		pmc_adm_render_ads( 'mobile-bottom-sticky-ad' );
	}
