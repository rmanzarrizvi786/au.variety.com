...
$embeddable = PMC_Ads::get_instance()->get_ad_property( 'embeddable', $ad );
$embeddable = empty( $embeddable ) ? 'No' : $embeddable;
...
<div class="adm-column-1">
	...
	<div class="adm-input">
		<label for="<?php echo esc_attr( $provider_id . '-embeddable' ); ?>">
			<strong><?php esc_html_e( 'Use location in Gutenberg block', 'pmc-adm' ); ?></strong>
		</label>
		<select name="embeddable" id="<?php echo esc_attr( $provider_id . '-embeddable' ); ?>">
			<?php foreach ( array( 'Yes', 'No' ) as $embeddable_condition ) : ?>
				<option
					value="<?php echo esc_attr( $embeddable_condition ); ?>"
					<?php selected( $embeddable, $embeddable_condition ); ?>>
					<?php echo esc_html( $embeddable_condition ); ?>
				</option>
			<?php endforeach; ?>
		</select>
	</div>
