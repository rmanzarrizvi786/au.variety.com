...
class PMC_Ads {
	...
	public $locations = array(
		...
		'mobile-bottom-sticky-ad'   => 'Mobile bottom sticky ad',
	);
	...
	public function add_lazy_load_cheezcap_group( $groups = array() ) {
		...
		$groups[] = new CheezCapGroup(
			...
			[
				...
				new CheezCapDropdownOption(
					__( 'Contextual Player on Direct-sold RBs', 'pmc-adm' ),
					__( 'Whether to remove or show contextual player on direct sold RBs ', 'pmc-adm' ),
					'pmc_contextual_on_direct_sold',
					array( 'remove', 'show' ),
					0, // 0 index => remove
					array( 'Remove', 'Show' )
				),
	...
	public function init() {
		$this->timezone = PMC_TimeMachine::get_site_timezone();
	...
	public function action_wp_enqueue_scripts() {
		if ( ! is_admin() ) {
			...
			if ( 'enable' === PMC_Cheezcap::get_instance()->get_option( 'pmc_enable_disable_page_level_referrer_targeting' ) ) {
			...
			$adm_config = [
				'contextual_on_direct_sold' => PMC_Cheezcap::get_instance()->get_option( 'pmc_contextual_on_direct_sold' ),
			];

			if ( false === $this->get_provider( 'boomerang' ) ) {
				// Enqueue ADM Loader JS
				wp_enqueue_script( 'pmc-adm-loader', plugins_url( sprintf( 'js/loader%s', $js_extension ), __FILE__ ), $adm_loader_deps, '5.3' );

				$adm_config = array_merge(
					[
			...
			wp_enqueue_script( 'pmc-adm-contextual-player', plugins_url( sprintf( 'js/contextual-player%s', $js_extension ), __FILE__ ), $adm_loader_deps, PMC_ADM_VERSION, true );

			/*
			 * Do not add any other pmc ad related setting.  Please add config to $adm_config instead.
			 */
			wp_localize_script( 'pmc-adm-contextual-player', 'pmc_adm_config', $adm_config );
			...
			wp_enqueue_style( 'pmc-adm-style', plugins_url( 'css/pmc-adm-style.css', __FILE__ ), array(), PMC_ADM_VERSION );
	...
	public function fetch_ads( $location = '', $title = '', $provider = '' ) {
		...
		$ad_posts = wp_cache_get( $cache_key, $this->cache_group() );
		...
		if ( empty( $ad_posts ) ) {
			...
			wp_cache_set( $cache_key, $ad_posts, $this->cache_group(), self::CACHE_LIFE );
		}
	 ...
	 * @return array
	 */
	public function sort_ads_by_priority( array &$ads ) : array {
		uasort(
			$ads,
			static function( $a1, $a2 ) : int {
				$p1 = $a1['priority'] ?? 10;
				$p2 = $a2['priority'] ?? 10;
				return $p1 <=> $p2;
			}
	 ...
	 * @return mixed
	 */
	public function get_ads_to_render( $ad_location, $ad_title = '', $provider = '' ) : array {
		// Fetch the ads post records
		// phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
		$ad_posts = apply_filters( 'pmc-adm-fetch-ads', $this->fetch_ads( $ad_location, $ad_title, $provider ) );
		...
		foreach ( $ad_posts as $post ) {
			...
			$group_key = $ad['width'] . 'x' . $ad['height'];

			if ( ! empty( $ad['slot-type'] ) ) {
				$group_key = $group_key . '-' . $ad['slot-type'];
			} else {
				$group_key = $group_key . '-normal';
			}

			if ( ! isset( $ads_groups[ $group_key ] ) ) {
				$ads_groups[ $group_key ] = [];
			}

			$ads_groups[ $group_key ][] = $ad;
		...
		// We can only serve one ad per slot size
		foreach ( $ads_groups as $key => $ads ) {
			$ads = $this->sort_ads_by_priority( $ads );
			// Need to pick the highest priority ad from the list per size
			$ads_unique[ $key ] = reset( $ads );
		}

		// We want to order the ads by their priority
		$ads_unique = $this->sort_ads_by_priority( $ads_unique );

		// Notes: We need normal ads render first before oop ads
		$normal = [];
		$oop    = [];

		foreach ( $ads_unique as $key => $ad ) {
	...
	public function render_ads( $ad_location, $ad_title = '', $echo = true, $provider = '' ) {
		...
		$provider = apply_filters( 'pmc_adm_display_provider_ads', $provider );
	...
	public function get_ads_based_on_role( $args = array() ) {
		...
		foreach ( (array) $current_user->roles as $role ) {
			...
			if ( 'administrator' === $role ) {
				$provider = apply_filters( 'pmc_adm_display_provider_ads', '' );
				$ads      = array_merge( $ads, $this->get_ads( false, '', $provider, $args ) );
				break;
		...
		try {
			...
			switch ( sanitize_title( $method ) ) {
				...
				case 'add':
					...
					$data['floating-player-id']          = PMC::filter_input( INPUT_POST, 'floating-player-id', FILTER_SANITIZE_STRING );
					$data['floating-player-media-id']    = PMC::filter_input( INPUT_POST, 'floating-player-media-id', FILTER_SANITIZE_STRING );
					$data['floating-player-playlist-id'] = PMC::filter_input( INPUT_POST, 'floating-player-playlist-id', FILTER_SANITIZE_STRING );
					...
					$data['contextual-player-media-id']     = PMC::filter_input( INPUT_POST, 'contextual-player-media-id', FILTER_SANITIZE_STRING );
					$data['contextual-player-playlist-id']  = PMC::filter_input( INPUT_POST, 'contextual-player-playlist-id', FILTER_SANITIZE_STRING );
					...
					// Entire method lacks coverage.
					$data['embeddable'] = PMC::filter_input( INPUT_POST, 'embeddable', FILTER_SANITIZE_STRING ); // @codeCoverageIgnore
	...
	public function save_post( $query, $data ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$embeddable_meta_key = '_ad_embeddable';
			if (
				! empty( $data['embeddable'] )
				&& 'Yes' === $data['embeddable']
			) {
				update_post_meta(
					$response,
				...
				delete_post_meta( $response, $embeddable_meta_key );
			}
	...
	public function get_ads( $cache = false, $location = '', $provider = '', array $args = [] ) {
		...
		$default_args = array(
			'numberposts'      => self::NUM_ADS_TO_FETCH,
			'post_type'        => self::POST_TYPE,
		...
		if ( $posts ) {
			...
			if ( $cache === true ) {
				...
				// phpcs:ignore WordPressVIPMinimum.Performance.LowExpiryCacheTime.CacheTimeUndetermined
				wp_cache_set( $cache_key, $posts, $this->cache_group(), self::CACHE_LIFE );
			}
		}
	...
	/**
	 * Helper function track current cache group to allow cache invalidation when ads updated
	 * @return string
	 */
	public function cache_group() : string {
		$instance = PMC_Options::get_instance( __CLASS__ );
		$group    = $instance->get_option( 'cache_group' );
		if ( empty( $group ) ) {
			$group = sprintf( 'cache-group-%d', time() );
			$instance->update_option( 'cache_group', $group );
		}
		return $group;
	}

	/**
	 * Helper function to invalidate the current cache by deleting the cache group data
	 * @return void
	 */
	protected function _invalidate_cache() {
		$instance = PMC_Options::get_instance( __CLASS__ );
		$instance->delete_option( 'cache_group' );
	}
