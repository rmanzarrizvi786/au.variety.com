...
class DS_Public_Post_Preview {
	...
	public static function get_preview_link( $post ) {
		...
		// Make sure nonce is not the last parameter.
		// The reason here is because we are doing a trailing slash and if the nonce is the last param,
		// it things the / is also part of the nonce and fails the nonce verification. 
		$args['_ppp']    = self::create_nonce( 'public_post_preview_' . $post->ID );
		$args['preview'] = true;

		// Adding a trailing slash here as part of BR-1264.
		// The public preview link results in a 404 without the trailing /.
		$link = trailingslashit( add_query_arg( $args, home_url( '/' ) ) );
