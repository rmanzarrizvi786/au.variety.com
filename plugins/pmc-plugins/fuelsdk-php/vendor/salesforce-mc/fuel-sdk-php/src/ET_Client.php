 ...
 */
	...
	function refreshToken($forceRefresh = false)
	{
		...
		try {
			$timeDiff = $this->getTokenExpireInMinutes();
	...
	* Function for basic field validation (present and neither empty nor only white space)
	...
	/**
	 * Update the current WSDL file
	 * @return string
	 */
	function setXmlLoc( $xmlLoc ) {
		$this->xmlLoc = $xmlLoc;
		return $this->getXmlLoc();
	}

	/**
	 * Helper function to return token expiration in seconds
	 * @return float|int|string
	 * @throws Exception
	 */
	function getTokenExpireInMinutes() {
		...
		return $timeDiff->i + ( $timeDiff->h * 60 );
	}
